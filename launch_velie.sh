...
# Python wrapper enforces socket-level timeout (60s TCP connect + 90s hard wall).
OCI="python3 /app/launch_helper.py oci"
...
while true; do
  ...
  if [ "$rc" -eq 0 ] && printf '%s' "$response" | grep -q '"lifecycle-state": *"\(RUNNING\|PROVISIONING\)"'; then
    ...
    log "[$timestamp] TIMEOUT (rc=$rc) - jaribio #$attempt. Retrying in 20s..."
    sleep 20
  else
    log "[$timestamp] ERROR - jaribio #$attempt (rc=$rc):"
